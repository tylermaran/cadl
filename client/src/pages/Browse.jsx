...
class Browse extends Component {
  ...
  update_results = () => {
    ...
    let search =
      process.env.REACT_APP_API_URL + "/projects/category/" + search_category;
    let category =
      process.env.REACT_APP_API_URL + "/categories/detail/" + search_category;
    let limit = "99";

    if (search_category === "recent") {
      search = process.env.REACT_APP_API_URL + "/projects/" + limit;
  ...
  render_model = object => {
    return <ResultsTile object={object} key={object._id} />;
  };
  ...
  render() {
    ...
    if (this.state.category && this.state.category === "recent") {
      categoryDetail = (
        <div className="browse_header">
          ...
          <div className="browse_title">Recent projects:</div>
        </div>
      );
    }
    if (this.state.category && this.state.category !== "recent") {
      browse_header = (
        <div className="browse_header">
          ...
          <div className="browse_title">Browse: {this.state.category.name}</div>
        </div>
      );
    ...
    let objectContainer = (
      <div className="no_results">
        No results found. You can help fix this!{" "}
